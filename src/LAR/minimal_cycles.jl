function minimal_2cycles(V::Common.Points, EV::Common.ChainOp)
    ...
    VE = convert(Common.ChainOp, SparseArrays.transpose(EV))
    EF = minimal_cycles(edge_angle)(V, VE)

    return convert(Common.ChainOp, SparseArrays.transpose(EF))
end
...
@show t;
    ...
    #EF = FE'
    EF = convert(Common.ChainOp, LinearAlgebra.transpose(FE))
		println(">>>>>>>> going to loop?")
Verbose = true
@show Verbose
		FC = minimal_cycles(face_angle, true)(V, EF)  # , EV)
...
println(">>>>>>>>>>>>> eccomi !!")
    ...
    function _minimal_cycles(V::Common.Points,
    		ld_bounds::Common.ChainOp)  # , EV)
...
if Verbose @show collect(enumerate(cop2lar(ld_bounds))) end
