...
annotations = PotreeConverter.potree2bim(potree; LOD=-1)

octLeav = Common.Struct(annotations.octreeLeaves)
V, EV, FV = Common.struct2lar(octLeav)

octPlane = Common.Struct(annotations.octreePlanes)
V_plane, EV_plane, FV_plane = Common.struct2lar(octPlane)

PC = FileManager.source2pc(potree,3)
...
Visualization.VIEW([
    ...
    Visualization.GLGrid(Common.apply_matrix(traslazione,V_plane),FV_plane,Visualization.COLORS[7]),
    Visualization.GLGrid(Common.apply_matrix(traslazione,V),EV,Visualization.COLORS[7]),
    ]);

Visualization.VIEW([
    # Visualization.GLGrid(Common.apply_matrix(traslazione,octree[1]),octree[2],Visualization.COLORS[7])
    Visualization.GLGrid(Common.apply_matrix(traslazione,W),EW, Visualization.COLORS[1],0.5)
    ])
...
node = raw"D:\\potreeDirectory\\pointclouds\\CASALETTO_TERRY_CONCLUSO_DECIMATO\\data\\r/r6020.las"
        #raw"C:\Users\marte\Documents\Julia_package\UTILS\potreeoriginale\MURI\data\r/r0.las"#collection[1]
